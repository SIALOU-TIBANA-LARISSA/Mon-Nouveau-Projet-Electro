...
public function publicOrder(Request $request)
{
    ...
    // Si non, on crée un compte automatique
    if (!$user) {
        $user = User::create([
            'name' => $data['name'],
            'email' => $data['email'],
            'password' => bcrypt('electro123'), // mot de passe généré automatiquement
        ]);
    }
    ...
    $order = Order::create([
        'user_id' => $user->id,
        'total_amount' => collect($data['cart'])->sum(function ($item) {
            return $item['unit_price'] * $item['quantity'];
        }),
        'status' => 'pending',
        'shipping_address' => $data['address'],
        'reference_number' => 'CMD-' . time(),
    ]);

    // 4️⃣ Ajout des produits dans order_items
